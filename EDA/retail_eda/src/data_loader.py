...
data_loader.py
--------------
Responsible for one thing only: return ing a clean pandas DataFrame.
...
def _generate_synthetic_data() -> pd.DataFrame:
    ...
    # Churn probability higher for short tenura, fewer purchase, many returns
    churn_prob = (
        0.05
        + 0.30 * (tenure < 12)
    ...
    # Inject ~3% missing value in a few columns to mimic real-world data quality
    for col in ["age", "annual_spend_usd", "last_pruchase_days"]:
        mask = rng.random(size=n) < 0.03
        df.loc[mask, col] = np.nan

    logger.info("Generated synthetic dataset: %d rows x %d columns.", * df.shape)
    return df
